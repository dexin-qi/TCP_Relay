...
class tcpRelay:
    ...
    def hasUserConfigOrder(self):
        ...
        if(configOrder=='exit'):
            ...
            self.connection.send((configOrder+"\n").encode('utf-8'))
    ...
    def writeRelay(self,state):
        if state:
            self.connection.send("AT+STACH0=1\n".encode('utf-8'))
        else:
            self.connection.send("AT+STACH0=0\n".encode('utf-8'))
    ...
    def getRelayStatus(self):
        self.connection.send("AT+STACH0=?\n".encode('utf-8'))
        relayBack=self.connection.recv(256)
        relayBack=relayBack.decode('utf-8')
...
class debugger:
    ...
    def getCmd(self):
        userCmd=self.connection.recv(1024).decode('utf-8')
